        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            @if($leases->count() > 0)
                ...
                <section class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                    <!-- Stat card -->
                    <div class="bg-white rounded-2xl shadow-md p-6 border-l-4 border-blue-500 hover:shadow-lg transition-shadow duration-300">
                    ...
                    <div class="bg-white rounded-2xl shadow-md p-6 border-l-4 border-green-500 hover:shadow-lg transition-shadow duration-300">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p class="text-3xl font-bold text-gray-900">{{ $leases->where('status', 'active')->count() }}</p>
                            </div>
                        </div>
                    </div>

                    <div class="bg-white rounded-2xl shadow-md p-6 border-l-4 border-yellow-500 hover:shadow-lg transition-shadow duration-300">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p class="text-3xl font-bold text-gray-900">{{ $leases->where('status', 'pending')->count() }}</p>
                            </div>
                        </div>
                    </div>
                </section>
                        ...
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-blue-50 transition duration-150">
                                    <td class="px-6 py-5">
                                        <div class="flex items-center space-x-4">
                                            <div class="h-12 w-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center shadow">
                                                <i class="fas fa-home text-white text-lg"></i>
                                            </div>
                                            <div>
                                                <p class="font-semibold text-gray-900">{{ $lease->property_unit }}</p>
                                    ...
                                    <td class="px-6 py-5">
                                        <span class="text-lg font-bold text-blue-600">₱{{ number_format($lease->rent_amount, 2) }}</span>
                                    ...
                                    <td class="px-6 py-5">
                                        @php
                                            $statusConfig = [
                                                ...
                                                'active' => ['color' => 'bg-green-100 text-green-800 border-green-200', 'icon' => 'check-circle'],
                                                'inactive' => ['color' => 'bg-red-100 text-red-800 border-red-200', 'icon' => 'times-circle'],
                                            ];
                                            $config = $statusConfig[$lease->status] ?? ['color' => 'bg-gray-100 text-gray-800 border-gray-200', 'icon' => 'question-circle'];
                                        @endphp
                                        <span class="px-3 py-2 inline-flex items-center text-sm font-semibold rounded-full border {{ $config['color'] }}">
                                    ...
                                    <td class="px-6 py-5">
                                        @php
                                            $daysRemaining = \Carbon\Carbon::parse($lease->lease_end)->diffInDays(\Carbon\Carbon::now());
                                            $isExpired = \Carbon\Carbon::parse($lease->lease_end)->isPast();
                                        @endphp
                                        @if($isExpired)
            ...
            @else
                ...
                <section class="bg-white rounded-2xl shadow-xl p-16 text-center border border-gray-100">
                    <div class="max-w-md mx-auto">
                        <div class="w-28 h-28 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-full flex items-center justify-center mx-auto mb-8 shadow-lg">
