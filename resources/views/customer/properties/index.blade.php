                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div>
                        <select name="bedrooms" onchange="this.form.submit()" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="any" {{ request('bedrooms') == 'any' ? 'selected' : '' }}>Any Bedrooms</option>
                    ...
                    <div>
                        <select name="bathrooms" onchange="this.form.submit()" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="any" {{ request('bathrooms') == 'any' ? 'selected' : '' }}>Any Bathrooms</option>
    ...
    <section class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white rounded-xl p-6 shadow-lg text-center">
                    <div class="text-2xl font-bold text-purple-600 mb-2">₱{{ number_format($properties->min('price') ?? 0) }} - ₱{{ number_format($properties->max('price') ?? 0) }}</div>
            ...
            <div class="text-center mt-12">
                <div class="bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-8">
                    ...
                    <a href="#" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg transition duration-300">
        ...
        delayTimer = setTimeout(() => {
            if (query.length >= 2 || query.length === 0) {
                $.ajax({
                    ...
                    data: { search: query },
                    beforeSend: function () {
                        $('#propertiesGrid').html('<div class="col-span-full text-center py-12 text-gray-500">Searching...</div>');
                    },
                    ...
                    error: function (xhr, status, error) {
                        ...
                        $('#propertiesGrid').html('<div class="col-span-full text-center py-12 text-red-500">Error occurred while searching. Please try again.</div>');
                    }
                });
            }
        }, 400);
